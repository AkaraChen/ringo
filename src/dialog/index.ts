...
export function dialog(property: DialogProperties): RingoInstance {
    ...
    const closeBtn = showClose
        ? el('i', { class: 'ringo-dialog-close', part: 'close' })
        : undefined;

    const btnGroup =
        buttons.length > 0
            ? el('div', { class: 'ringo-dialog-btns', part: 'buttons' })
            : undefined;

    const panel = el('div', { class: 'ringo-dialog', part: 'panel' }, [
        el('div', { class: 'ringo-dialog-head' }, [
            el('p', { class: 'ringo-dialog-title', part: 'title' }, [title]),
            closeBtn
        ]),
        el('div', { class: 'ringo-dialog-body', part: 'content' }, [text]),
        btnGroup
    ]);
    shadow.append(panel);
